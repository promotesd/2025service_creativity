...
# 输出文件，用于存储多轮对话JSON
OUTPUT_FILE = r"/root/autodl-tmp/code/2025service_creativity/process_dataset/huggingfaceset/GoT_orca_math_word_problems.json"

# ========== 1. 加载你的数据集 ==========
# 这里你想用 "xd2333___orca-math-word-problems-100k-en-zh-mix"
dataset_all = load_dataset(r"/root/.cache/huggingface/datasets/xd2333___orca-math-word-problems-100k-en-zh-mix")
print(dataset_all)  # 查看数据集信息

ds = dataset_all["train"]

# 该数据集的字段为 ['instruction', 'output', 'system']，其中：
# - instruction: 题目
# - output: 解答
# - system: 一些系统信息（可忽略或备用）
...
def generate_got_json(prompt_text: str) -> dict:
    ...
    with torch.no_grad():
        output_ids = model.generate(
            **inputs,
            max_new_tokens=1024,
            do_sample=False,  # 可改为True并设置 temperature 等超参
        )
...
def main():
    ...
    for i, row in enumerate(ds):
        # 该数据集没有专门的"id"字段，你可以用i来生成唯一ID
        record_id = f"orca-{i+1}"

        # instruction 是题目
        user_input = row["instruction"]
        # output 是解答，这里既可当“参考输出”，也可当“参考解答”
